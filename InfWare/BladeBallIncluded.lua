...
local Distance -- Declare the variable here
...
UserInputService.InputBegan:Connect(function(Input, IsTyping)
    ...
    if Input.KeyCode == shared.config.keybind then
        ToggleScript()
    end
end)
...
local Toggle = BladeBall:CreateToggle({
...
local Button = BladeBall:CreateButton({
   Name = "Load Swapper [ABILITYS]",
   Callback = function()
 local localPlayer = game.Players.LocalPlayer
local character = localPlayer.Character or localPlayer.CharacterAdded:Wait()
local abilitiesFolder = character:WaitForChild("Abilities")
...
local function createGUI()
    ...
    local infRecLabel = Instance.new("TextLabel")
    infRecLabel.Size = UDim2.new(1, 0, 0, 30)
    infRecLabel.Position = UDim2.new(0, 0, 0, 0)
    infRecLabel.Text = "Inf-Ware [SWAPPER]" -- Updated label
    infRecLabel.Font = Enum.Font.SourceSansBold
    infRecLabel.TextSize = 24
    infRecLabel.TextColor3 = Color3.new(1, 1, 1)
    infRecLabel.BackgroundTransparency = 1
    infRecLabel.Parent = frame
    ...
    local abilityListFrame = Instance.new("Frame")
    abilityListFrame.Size = UDim2.new(1, 0, 1, -30)
    abilityListFrame.Position = UDim2.new(0, 0, 0, 30)
    abilityListFrame.Parent = frame

    local abilityListLayout = Instance.new("UIListLayout")
    abilityListLayout.Parent = abilityListFrame
    abilityListLayout.SortOrder = Enum.SortOrder.LayoutOrder

    local abilityButtons = {}
    local abilities = {
        "Dash", "Forcefield", "Invisibility", "Platform", "Raging Deflection", "Shadow Step",
        "Super Jump", "Telekinesis", "Thunder Dash", "Freeze", "Infinity", "Pull", "Rapture",
        "Phase Bypass", "Waypoint", "Swap"
    }
    ...
    for _, ability in ipairs(abilities) do
        ...
        button.Parent = abilityListFrame

        button.MouseButton1Click:Connect(function()
            ChosenAbility = ability
        end)

        table.insert(abilityButtons, button)
    end

    minimizeButton.MouseButton1Click:Connect(function()
        isMinimized = not isMinimized
        frame:TweenSize(UDim2.new(0, 30, isMinimized and 0 or 0.4, isMinimized and 30 or 0), "Out", "Quad", 0.2, true)
    end)
