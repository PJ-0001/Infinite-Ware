...
local Info = Window:CreateTab("Information", nil) -- Title, Image
...
local DaHood = Window:CreateTab("Da-Hood Cheats", nil) -- Title, Image

local Button = Info:CreateButton({
    Name = "Developer: PJ0001",
    Callback = function()
...
local Toggle = DaHood:CreateToggle({
    Name = "Auto Farm",
    CurrentValue = false,
    Flag = "Toggle9",
    Callback = function(Value)
...
local function toggleScript()
    ...
    for i, v in ipairs(LP.Character:GetDescendants()) do
        if v:IsA("MeshPart") then
            v.Massless = _G.ScriptEnabled
            v.CanCollide = not _G.ScriptEnabled
            v.CustomPhysicalProperties = _G.ScriptEnabled and PhysicalProperties.new(0, 0, 0, 0, 0) or nil
        end
    end

    for i, v in ipairs(game.workspace:GetDescendants()) do
        if v:IsA("Seat") then
            v.Disabled = _G.ScriptEnabled
        end
    end

    local x, y, z = 35, 35, 35
    local penis = Vector3.new(x, y, z)

    LP.Character.RightHand.Size = _G.ScriptEnabled and penis or Vector3.new(1, 1, 1)
    LP.Character.RightHand.Transparency = _G.ScriptEnabled and 1 or 0

    local selectionBox = LP.Character.RightHand:FindFirstChild("SelectionBox")
    if _G.ScriptEnabled then
        if not selectionBox then
            selectionBox = Instance.new("SelectionBox")
            selectionBox.Parent = LP.Character.RightHand
    ...
    LP.Character.LeftHand.Size = _G.ScriptEnabled and penis or Vector3.new(1, 1, 1)
    LP.Character.BodyEffects.SpecialParts.LeftHand.Size = _G.ScriptEnabled and penis or Vector3.new(1, 1, 1)
    LP.Character.LeftHand.Transparency = _G.ScriptEnabled and 1 or 0

    local leftSelectionBox = LP.Character.LeftHand:FindFirstChild("SelectionBox")
    if _G.ScriptEnabled then
        if not leftSelectionBox then
            leftSelectionBox = Instance.new("SelectionBox")
            leftSelectionBox.Parent = LP.Character.LeftHand
            leftSelectionBox.Adornee = LP.Character.LeftHand
            leftSelectionBox.Color3 = Color3.new(1, 0, 0)
            leftSelectionBox.Name = "SelectionBox"
        end
    else
        if leftSelectionBox then
            leftSelectionBox:Destroy()
        end
    end
end
